...
def _build_pipeline(model, *, scale_numeric: bool) -> Pipeline:
    return Pipeline(
        steps=[
...
def train(dataset_path: str = CFG.dataset_path, output_dir: str = "artifacts") -> None:
    ...
    for name, metrics in baseline_eval.items():
        _save_confusion(
            ...
            plots_dir / f"{name.lower().replace(' ', '_')}_baseline_cm.png",
        )
    for name, metrics in tuned_eval.items():
        _save_confusion(
            metrics["cm"],
            f"Confusion Matrix - {name} (Tuned)",
            plots_dir / f"{name.lower().replace(' ', '_')}_tuned_cm.png",
        )
    ...
    _save_bar(
        ...
        path=plots_dir / "top2_tuned_comparison.png",
    )
    ...
    plt.tight_layout()
    plt.savefig(plots_dir / "missing_values_by_feature.png", dpi=180)
    plt.close()
    ...
    # Reports
    baseline_table.to_csv(reports_dir / "baseline_model_performance_table.csv", index=False)
    tuned_table.to_csv(reports_dir / "tuned_top_models_performance_table.csv", index=False)
    comparison_table.to_csv(reports_dir / "top2_model_comparison.csv", index=False)
    tuned_table.to_csv(reports_dir / "model_performance_table.csv", index=False)
    (reports_dir / "eda_summary.json").write_text(json.dumps(eda, indent=2), encoding="utf-8")
